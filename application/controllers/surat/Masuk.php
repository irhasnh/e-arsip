...
class Masuk extends CI_Controller {
    ...
    {
        $this->template->authlogin('r',2,'Y');
        $permission = $this->template->get_sidebar_data();
        ...
        foreach ($list as $u) {
            ...
            if ($permission['update'] == 1) {
                // code...
                $update = "<button class='btn btn-xs btn-primary btn-flat' data-toggle='tooltip' data-placement='top' title='edit data' onclick=\"edit('".$id."')\"><i class='fa fa-edit'></i></button> <button class='btn btn-xs btn-success btn-flat' data-toggle='tooltip' data-placement='top' title='upload file' onclick=\"upload('".$id."')\"><i class='fa fa-upload'></i></button> ";
            }
            $row[] = $update;
            $data[] = $row;
            $yey = $yey+1;
